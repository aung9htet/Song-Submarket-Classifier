...
class RunDataProcess():
            ...
            with open(csv_file, mode='a', newline='', encoding='utf-8') as file:
                ...
                for track in tracks:
                    try:
                        ...
                        song_data = self.process_data_dict(cyanite_data, spotify_data, track_playcount, composer_name, chosen_song)
    ...
    def process_data_dict(self, cyanite_data, spotify_data, track_playcount, composer_name, org_song):
        """
        ...
        song_data = {'Song_Name': spotify_data['name'], 'Song_Link': spotify_data['id'], 'Artist_Name': ', '.join([item['name'] for item in spotify_data['artists'] if 'name' in item]), 
                  ...
                  'Movement_Tags': None if cyanite_data['Movement_Tags'] is None else ','.join(cyanite_data['Movement_Tags']), 'Energy': cyanite_data['Energy'], 'BPM': None if cyanite_data['BPM'] is None else cyanite_data['BPM']['value'], 'Key': None if cyanite_data['Key'] is None else cyanite_data['Key']['value'], 'Meter': cyanite_data['Meter'], 'Composer Nmae': composer_name, 'Original Song Estimate': org_song}
        return song_data
